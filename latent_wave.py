...
def generate_prey_pred(start=0, stop=1, niter=1000, ntotal=500, nsample=100,
                        a=1., b=0.1, c=1.5, d=0.75, noise_std=.1):
    ...
    orig_ts = np.linspace(start, stop,  num=ntotal)              # time
    samp_ts = orig_ts[:nsample]
    X0 = np.array([10,5])                     # initials conditions: 10 rabbits and 5 foxes

    X, infodict = integrate.odeint(dX_dt, X0, orig_ts, full_output=True)
    orig_traj = X

    orig_traj = (orig_traj - np.mean(orig_traj,axis = 0))/np.std(orig_traj, axis = 0)
...
if __name__ == '__main__':
    ...
    #     a=a, b=b
    # )
    orig_ts = torch.from_numpy(np.arange(0,10,0.1)).to(device)
    samp_ts = orig_ts
    samp_trajs = torch.from_numpy(data).to(device)
    orig_trajs = torch.from_numpy(data).to(device)
    # orig_trajs, samp_trajs, orig_ts, samp_ts = generate_prey_pred(start = start, stop = stop)
    ...
    rec = RecognitionRNN(latent_dim, obs_dim, rnn_nhidden, niter).to(device)
    ...
    try:
        ...
        # for itr in range(2):
            ...
            print('Iter: {}, running avg elbo: {:.4f}, nfe: {}'.format(itr, -loss_meter.avg, func.nfe))
            ...
            if args.visualize and itr%20==0:
                with torch.no_grad():
                    ...
                    zs_pos = odeint(func, z0, samp_ts).permute(1, 0, 2)
                    ...
                    samp_traj = samp_trajs.cpu().numpy()
                    # plt.figure()
                    ...
                    rchoice = np.random.choice(1000, 16, replace = False)
                    # for i in range(4):
                    #     for j in range(4):
                    #         ax[i, j].plot(ts, orig_traj[(i)*(2)+j], 'b')
                    #         ax[i, j].plot(ts, orig_traj[(i)*(2)+j], 'r')
                    for i in range(4):
                        for j in range(4):
                            ax[i, j].plot(ts, orig_traj[rchoice[(i)*(2)+j]], 'b')
                            ax[i, j].plot(ts, xs_pos[rchoice[(i)*(2)+j]], 'r')
                    ...
                    plt.legend()
                    plt.savefig('./vis'+str(itr)+'.png', dpi=500)
